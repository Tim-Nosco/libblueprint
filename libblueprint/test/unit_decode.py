#!/usr/bin/env python3
# coding: utf-8

import sys, ctypes
from ctypes import c_uint32, c_char_p, c_void_p

def test1(f):
    data = """""".encode('utf-8')
    return f(data,len(data))

def from_file(filename):
    #todo: make bytes from file for correct encodings
    with open(filename, "rb") as f:
        data = f.read()
    return data, len(data)

def call_lib(lib,func_name,*args):
    r = getattr(lib,func_name)(*args)
    try:
        final = ctypes.cast(r, ctypes.c_char_p).value.decode('utf-8')
        return final
    finally:
        lib.free_return(r)

def setup_lib():
    lib = ctypes.cdll.LoadLibrary('target/debug/libblueprint.so')
    lib.decode_to_json.argtypes = (c_char_p,c_uint32)
    lib.decode_to_json.restype = c_void_p
    lib.decode_to_grid.argtypes = (c_char_p,c_uint32)
    lib.decode_to_grid.restype = c_void_p
    lib.free_return.argtypes = (c_void_p, )
    return lambda f,*args: call_lib(lib,f,*args)

if __name__ == "__main__":
    test_func = setup_lib()
    from functools import partial
    #get json
    decoded = test1(partial(test_func,'decode_to_json'))
    #convert to grid
    bp = test_func('decode_to_grid', 
                    decoded.encode('utf-8'), 
                    len(decoded))
